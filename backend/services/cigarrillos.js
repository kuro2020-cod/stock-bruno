...
export const HORA_CORTE_CIGARRILLOS = HORA_CORTE_MILANESAS;
export const CODIGO_PRODUCTO_REINICIO_CIGARRILLOS = 'REINICIO-CIGARRILLOS';

/**
 * Condición SQL: cigarrillos (nombre o categoría).
 * Alias esperados: productos p, categorias c (LEFT JOIN).
 */
export const SQL_CIGARRILLOS = `(
  LOWER(p.nombre) LIKE '%cigarr%'
  OR LOWER(COALESCE(c.nombre, '')) LIKE '%cigarr%'
)`;
...
async function asegurarProductoReinicio(tx = db) {
  let prod = await tx.get(
    ...
    LIMIT 1
  `,
    [CODIGO_PRODUCTO_REINICIO_CIGARRILLOS]
  );
  if (prod) return prod;
  ...
  `,
    [
      CODIGO_PRODUCTO_REINICIO_CIGARRILLOS,
      'Reinicio contador cigarrillos',
      'Registro diario del total vendido en cigarrillos (no es venta)'
    ]
  );
  return { id: ins.lastID, nombre: 'Reinicio contador cigarrillos' };
}

async function sumarVentas({ desdeExclusivo = null, hastaInclusive = null } = {}) {
  const params = [CODIGO_PRODUCTO_REINICIO_CIGARRILLOS];
  let condFecha = '';
...
export async function asegurarReinicioJornadaCigarrillos() {
  const corte = inicioJornadaMilanesas();
  const corteIso = corte.toISOString();
  const ultimo = await getUltimoReinicioCigarrillos();

  if (!ultimo) {
    await db.run(
      `
      INSERT INTO cigarrillos_reinicios
  ...
  await db.transaction(async (tx) => {
    ...
    const motivo =
      montoAntes > 0
        ? `Reinicio contador cigarrillos (corte 06:00): total vendido $${fmtMoneyMotivo(montoAntes)} (${unidadesAntes} u.); contador vuelve a 0.`
        : 'Reinicio contador cigarrillos (corte 06:00): sin ventas en la jornada; contador vuelve a 0.';
...
export async function getContadorCigarrillosJornada() {
  await asegurarReinicioJornadaCigarrillos();
  ...
  return {
    ...
    horaCorte: HORA_CORTE_CIGARRILLOS
  };
}

export async function getHistorialReiniciosCigarrillos(limit = 14) {
...
export async function getResumenCigarrillos({ historialLimit = 14 } = {}) {
  const actual = await getContadorCigarrillosJornada();
  const historial = await getHistorialReiniciosCigarrillos(historialLimit);
  const ultimoReinicio = await getUltimoReinicioCigarrillos();
